...
def test(dataset, params):
    ...
    for batch_count, batch in enumerate(batches):
        ...
        pred_texts.extend(get_raw_texts(output_tokens, vocab=dataset.vocab, special_tokens=dataset.SPECIAL_TOKENS))
        target_texts.extend(get_raw_texts(target_tensor, vocab=dataset.vocab, special_tokens=dataset.SPECIAL_TOKENS))
        source_texts.extend(get_raw_texts(source_tensor, vocab=dataset.vocab, special_tokens=dataset.SPECIAL_TOKENS))
        if batch_count % 100 == 0:
